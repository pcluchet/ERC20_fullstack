...
### CC erc20
	...
	docker exec  -e "CORE_PEER_ADDRESS=peer1.HSLU.example.com:7051" -e "CORE_PEER_LOCALMSPID=HSLUMSP" -e "CORE_PEER_MSPCONFIGPATH=/opt/gopath/src/github.com/hyperledger/fabric/peer/crypto/peerOrganizations/HSLU.example.com/users/Admin@HSLU.example.com/msp" cli peer chaincode install -n ERC20 -v 1.0 -p "$ERC20_CC_SRC_PATH" -l "$LANGUAGE"
	

	docker exec  -e "CORE_PEER_ADDRESS=peer0.BLUECITY.example.com:7051" -e "CORE_PEER_LOCALMSPID=BLUECITYMSP" -e "CORE_PEER_MSPCONFIGPATH=/opt/gopath/src/github.com/hyperledger/fabric/peer/crypto/peerOrganizations/BLUECITY.example.com/users/Admin@BLUECITY.example.com/msp" cli peer chaincode install -n ERC20 -v 1.0 -p "$ERC20_CC_SRC_PATH" -l "$LANGUAGE"
	
	
	docker exec  -e "CORE_PEER_ADDRESS=peer1.BLUECITY.example.com:7051" -e "CORE_PEER_LOCALMSPID=BLUECITYMSP" -e "CORE_PEER_MSPCONFIGPATH=/opt/gopath/src/github.com/hyperledger/fabric/peer/crypto/peerOrganizations/BLUECITY.example.com/users/Admin@BLUECITY.example.com/msp" cli peer chaincode install -n ERC20 -v 1.0 -p "$ERC20_CC_SRC_PATH" -l "$LANGUAGE"
